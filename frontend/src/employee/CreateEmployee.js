...
import {createEmployee} from "../actions/apiMethods";
...
const CreateEmployee = () =>{
    ...
    const clickSubmit = (event) =>{
        ...
        createEmployee({ name, salary, address, gender, team },token,user._id)
        .then(data=>{
            if(data.error){
            ...
            <form>
                <div className = "row">
                    <div className = "form-group col-9">
                        ...
                        <input className = "form-control" type = "text" value = {name} onChange = {handleChange("name")} required />
                    </div>
                    <div className = "form-group col-3">
                        <label className = "text-muted">Salary:</label>
                        <input className = "form-control" type = "text" value = {salary} onChange = {handleChange("salary")} required/>
                    </div>
                </div>
                ...
                <div className="form-check">
                    <input className="form-check-input" type="radio" name="gender" value = "male" checked={gender === "male"} onChange={handleChange("gender")} required/>
    ...
    return (
        <Layout title = "Create Employee" description = "Fill in the Employee details." className = "container col-md-8 offset-md-2">
            {showSuccess()}
            {showError()}
            {employeeForm()}
        </Layout>
